function [StressDisp] = ...
Quad_coeff_funcOld(x,y,xe,ye,a,Beta,Sd,Nd,pr,G)
...
%Nd - normal disp, 1x3 mat
%Sd - Shear disp, 1x3 mat
%Poisson's ratio - Pr
%Shear mod - G
...
[xloc,yloc] = RotateObject2d(xloc,yloc,Beta);

%Preallocating arrays
UxA=zeros(size(xloc));UyA=UxA;
SxxA=UxA;SyyA=UxA;SxyA=UxA;
...
for i=1:numel(xloc)
%x location of point relative to frac
x=xloc(i);
%y location of point relative to frac
y=yloc(i);
...
%Ux=sum((2*(J.*Dx))-k2*(I.*Dy)-k1*y*((Ix.*Dx)+(Jy.*Dx)));
%Uy=sum((2*(J.*Dy))+k2*(I.*Dx)-k1*y*((Ix.*Dy)+(Jy.*Dy)));
%dot form (slower):
%Ux=dot((-2*J+k1*y*Ix),Dx)+dot((k2*I+k1*y*Iy),Dy);
%Uy=dot((-k2*I+k1*y*Iy),Dx)+dot((-2*J+k1*y*Jy),Dy);
...
%dot form (slower):
% Sxx=-k3*(dot((2*Iy+y*Iyy),Dx)+dot((Jy+y*Jyy),Dy));
% Syy=-k3*(-y.*dot(Iyy,Dx)+dot((Jy-y*Jyy),Dy));
% Sxy=-k3*(dot((Jy+y*Jyy),Dx)-y.*dot(Iyy,Dy));
...
%Now extracting points, appending to a master vec
UxA(i)=-Ux;
UyA(i)=-Uy;
SxxA(i)=Sxx;
SyyA(i)=Syy;
SxyA(i)=Sxy;
 ...
 StressDisp=[SxxA(:),SyyA(:),SxyA(:),UxA(:),UyA(:)];
