...
flag = 2;
    ...
    ylabel('DssElDisp+'); 
    h3=subplot(5,2,5);
    ylabel({'Avg Calculated FB function in comp formulation';'DdsElDisp+'}); 
    h4=subplot(5,2,7);
    ylabel('tssElDisp-');
    h5=subplot(5,2,9);  
    ylabel('tdsElDisp-');
...
while (iter <= max_iter )
    ...
    switch lower(solver)
        ...
        case 'zero'   % works on reduced system would be similar to random-case if all radom values were set to 1
            ...
            J=sparse(J);               %sparse for later calcs, would be great to allocate properly
            ...
            %dx(I)=J(I,I)\-phi(I); On big problems with dense mats the 
            %gmres approach is far faster at finding a minimum (for me)
            %note the original code comes with the comment 'Or one can hope for the best and try GMRES'
            ...
            %%the original code, works but not as fast when working with the dense
   ...
   if display        
   [yD1,xD1,yD2,xD2,yD3,xD3,yD4,xD4,yD5,xD5,timed,psy]=drawprep(x,phi_k,f_k,psy,timed,h1,h2,h3,h4,h5,h6,iter,max_iter,yD1,xD1,yD2,xD2,yD3,xD3,yD4,xD4,yD5,xD5,N);
...
function [yD1,xD1,yD2,xD2,yD3,xD3,yD4,xD4,yD5,xD5,timed,psy]=drawprep(x,y,f_k,psy,timed,h1,h2,h3,h4,h5,h6,iter,max_iter,yD1,xD1,yD2,xD2,yD3,xD3,yD4,xD4,yD5,xD5,sz);
  
%setting up variables for the sub functions 'draw' and 'draw2' below

    %3d comp formulation of kaven 2015, so splitting vector into 5 parts and calculating means
